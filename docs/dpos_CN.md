...
|事件     |输出                          |说明 |
| :----- | ---------------------------- | -------------- |
|apply   |`申请者地址`、`申请角色`、 `投票奖励分配地址`、 `投票奖励分配比率`、 `物理节点地址`       |用户申请委员会委员或竞选节点时触发的事件。如果申请角色为委员会委员，只打印申请者地址和申请角色，如果为kol，除了打印前两者，还将打印投票奖励分配地址和比率，validator打印全部信息。|
|abolish|`举报者地址`、`被举报者角色`、`被举报者地址`、`举报证据`|委员会委员或竞选节点被举报时触发的事件。|
|configure|`提案者`、`配置项`、`配置值`|委员会委员提案修改某项选举配置时触发的事件。|
|approved|`提案动作`、`提案项目`、`被提案者或提案者地址`|申请提案审核通过后触发的事件。提案动作为[提案动作](#提案动作)章节中列出的值之一；提案项目为[用户角色](#用户角色)章节或[选举配置](#选举配置)章节列出的值之一；最后一个输出为被提案者地址，如果是修改选举配置提案，则为提案者地址。|
|append  |`追加者地址`、`追加者角色`、`追加金额`|竞选节点追加质押金时触发的事件。|
|withdraw|`退出者地址`、`角色`、`质押金`|委员会成员或竞选节点推出所在序列时触发的事件。需要注意的是，委员会委员和竞选节点退出所在序列时不打印质押金，但竞选节点的质押金锁定期结束后，再次触发退出操作收回质押金时，将打印质押金数额。|
|vote|`投票者地址`、`被投票者角色`、`被投票者地址`、`投票数`|用户为某个竞选节点投票时触发的事件。|
|unVote|`撤销投票者地址`、`被撤销投票者角色`、`被撤销投票者地址`、`撤销投票数`|用户撤销对某个竞选节点所投的票时触发的事件。|
|addCandidate|`被加入者地址`、`角色`|申请者成功加入委员会或竞选列表时触发的事件。|
|deleteCandidate|`被删除者地址`、`角色`|委员会委员或竞选节点从所在的候选列表删除时触发的事件。|
|updateStake|`竞选者地址`、`角色`、`权益变更值`|竞选节点的权益变更时触发的事件。审核通过、追加质押金、投票、撤销投票等都有可能触发此事件。另外，权益变更值可以为负数，因为撤销投票行为将触发权益值减少。|
|updateValidators|`验证节点集合大小`|记账节点集合更新时触发的事件。|
|updateConfigure|`配置项`、`配置值`|选举相关配置变更时触发的事件。|
|penalty|`作恶者地址`、`角色`、`罚没数额`|作恶节点质押金被罚没时触发的事件。|
|setNodeAddress|`记账节点资金地址`、`新物理节点地址`|记账节点设置物理节点地址时触发的事件。|
|setVoteDividend|`设置者角色`、`新投票奖励分配地址`、`新投票奖励分配比率`|竞选节点设置投票奖励分配地址或比率时触发的事件。|
|rewardDistribution|`分配奖励总额`、`触发者地址`|区块奖励计算分配时触发的事件。竞选节点变更和用户主动提取奖励都会触发此事件。|
|award|`收益者地址`、`预留奖励数额`、`投票奖励分配地址`、`投票者分得的奖励数额`|分配后的奖励被提取时触发的事件。如果投票奖励分配比率为0，那么只打印收益者地址和预留奖励数额，因为全部奖励都转账给了收益者地址。如果投票奖励分配比率为100，那么只打印投票奖励分配地址和投票者分得的奖励数额，因为全部奖励都转账给了投票奖励分配地址。其余情况打印全部输出。|
|clean|`提案动作`、`提案项目`、`被提案者或提案者地址`|清理过期提案时触发的事件|
