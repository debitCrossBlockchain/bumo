...
|方法     |参数                          |说明 |
| :----- | ---------------------------- | -------------- |
|apply   |`role`                        |申请接口，任意BuChain账户申请成为候选验证节点、候选KOL和委员会委员。`role`参数为申请的角色，参数值必须为[用户角色](#用户角色)章节中列出的值之一。|
...
|unVote   |`role`、`address`、`amount`  |撤销投票接口。已投票账户撤回部分或全部投出的选票。`amount`参数为撤回的选票数。`role`参数和`address`参数解释同上。|
...
- 向DPOS合约转账 0 BU。
- ‘转移货币’操作的 input 字段填入`{ "method" : "unVote", "params" : { "role":"validator", "address" : "填入候选节点地址", "amount": "填入减少的投票数"} }`，注意使用转义字符。
...
- 一年的区块奖励总收益：8640* 8* 0.3* 365/天 = ￥7568640
- 验证节点、候选节点、KOL按7:2:1分配奖励。
...
- DAPP：100w tx* 0.0025* 0.3* 0.7（DAPP获得70%的交易费激励） = 525， 100w交易获得￥525的返现。
