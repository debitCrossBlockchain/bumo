...
### 角色和动作类型
DPOS 合约中涉及多种角色和动作类型，不同角色可执行的动作不同，用户可以通过特定的动作为自己添加角色。

#### 角色(role)

任意 BuChain 账户可以参选委员会委员(committee)或KOL(Key Opinion Leader)，拥有节点的账户还可以参选验证节点(validator)。对BuChain账户来说，下文 roles 数组内列出的角色都可以申请，且一个账户可申请和担任多种角色。
...
数组roles内列出的角色字符串，主要用于DPOS合约在执行申请(apply接口)、退出(withdraw接口)、废止(abolish接口)和审核(approve接口)操作时，作为接口的参数，以区别不同的角色类型。

#### 动作(motion)
任意BuChain账户可以执行apply动作，申请成为某一种[角色(role)](#角色(role))；abolish动作可以举报某个不称职的同角色成员并生成废止提案；withdraw动作可退出账户所在的角色集合；委员会成员可以执行config动作提议更改配置。
DPOS合约支持的动作类型如下所示：
...
数组motions内列出的动作字符串，主要用于DPOS合约在执行审核(approve接口)时，作为接口的参数，以区别提案的类型。
...
注：委员会对所有提案和申请审核的原则是，一人一票，支持则投票，不支持则不投票，重复投票视为一票。在审核有效期内票数超过投票通过率，则视为审核通过。如果截止到审核期结束，票数未超过投票通过率，则视为审核未通过。
...
任何BuChain账户可以通过向DPOS合约转移一笔 BU 作为押金，申请成为委员会委员、候选验证节点或候选KOL，质押金额参考[选举配置](#选举配置)，如果是申请委员会委员，不需要质押金。经委员会投票审核（参考[委员会新成员批准投票](#委员会新成员批准投票)）通过后，可成为正式的候选节点。但能否成为验证节点，是根据一定周期内获得的总票数决定的。
