...
- 申请者向DPOS合约转移一笔 BU 作为押金（参见开发文档```转移BU资产```），用户如果退出，该质押金将被锁定30天，超过锁定期后，再次申请退出，合约会将质押金全部返还申请账户（参见[候选共识节点退出](#候选共识节点退出)）。
...
- 申请者向DPOS合约转移一笔 BU 作为押金（参见开发文档```转移BU资产```），该押金可通过 ‘[生态节点退出](#生态节点退出)’ 操作收回。
