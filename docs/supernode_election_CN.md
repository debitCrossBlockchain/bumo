...
Bumo致力于打造价值流通基础公链网络，价值流通的基础是公平，公正，公开。为回馈广大用户，展现BU token的价值投资属性，
提高公链去中心化程度，Bumo启动了超级节点计划。我们希望超级节点计划能帮助达成完善商用级基础公链以及扩大社区规模的目的。
...
|角色|说明|
| :----- | ------------------------ |
|用户|Bu持有者，在Bumo上拥有唯一账户，可参与竞选，投票，质押金众筹。|
|共识节点|Firework中的参与共识、打包区块的节点，需要独立的服务器参与分布式账本记账。|
...
1. `保证资金安全`，共识节点资金账户与参与共识的节点地址分开，节点服务器上不保存资金相关的私钥信息，保证资金安全。
...
|生态节点集合数目          |21|
|生态节点候选人集合数目     |100|
|生态节点候选人最小质押金额 |5000000000000 (500万BU)|
|共识节点集合数目          |19|
|共识节点候选人集合数目    |100|
|共识节点候选人最小质押金额|500000000000000 (500万BU)|
...
|共识节点和生态节点奖励分配比|6 (代表共识节点集合平分区块奖励的60%，生态节点集合平分区块奖励的40%)|
|超级节点和候选节点奖励分配比|9 (当选的超级节点获取90%的奖励，10%的收益分给候选节点)|
...
> 1. 参与超级节点的候选人至少抵押500万BU（共识节点和生态节点最少质押金均为500万BU）。
> 2. 共识节点候选人需要提供符合条件的软硬件和网络环境，生态节点候选人需提供公众影响力证明。
> 3. 候选人须填写申请材料，并保证申请材料的真实有效。
...
每个区块的挖矿收益和交易费将通过一定的比例分配给超级节点和候选人。
当前涉及的分配角色包含共识节点及其候选人，生态节点及其候选人。比例为5:4:1，共识节点50%，生态节点40%，剩余10%分给候选人。
...
> 1. 一年的区块奖励：(365 * 24 * 3600 / 10) * 8 = 25228800
> 2. 共识节点（以19个为例）：25228800 * 0.6 * 0.9 / 19 = 717029.1
> 3. 生态节点（以21个为例）：25228800 * 0.4 * 0.9 / 21 = 432493.7
> 4. 候选生态节点（以10个为例）：25228800 * 0.1 / 20 = 126144
> 5. 以质押500万BU为例，共识节点的年收益约为：14.34%，生态节点的年收益余约为：8.65%，候选共识节点的收益约为：2.52%
> 6. 交易费用的分配随着时间的累计会不断增加，当前以太坊每日交易量约为50万笔，假设一天交易量为50万笔，按每笔交易0.0025BU的交易费计算：365 * 500000 * 0.0025 = 456250， 交易费的20%作为区块奖励分给超级节点和候选人，约有91250 BU参与交易分配。
> 7. 质押金可以申请撤回，年化收益率不随代币价格而变化，属于保本的固定收益型理财。

### 节点共建以及收益分配

考虑到质押金500万BU对于大多数的普通用户来说较多，Bumo提出了节点共建方案，申请者在满足一定条件下可以通过节点共建合约来参与竞选。

- 以下是一个节点共建合约的执行流程：

> 1. 申请者向节点共建合约转入一定数量BU作为原始股份。
> 2. 申请者邀请众多好友入股或者公开招股的方式筹集资金。
> 3. 资金达标后触发节点共建合约执行申请候选人操作。
> 4. 申请通过后进入候选人列表接受投票。
> 5. 候选人排名靠前成为超级节点，开始累计收益。
> 6. 申请者领取收益触发收益领取操作，将收益领取到智能合约并自动为每个参与者累计收益数值。
> 7. 参与者在收益累计到一定程度后从节点共建合约领取自己的收益。