...
function clean(operate,item,address){Utils.assert(operateValid(operate),'Unknown proposal operation:'+operate+'.');Utils.assert(roleValid(item)||cfg[item]!==undefined,'Unknown proposal item:'+item+'.');Utils.assert(Utils.addressCheck(address),'Invalid address:'+address+'.');let key=proposalKey(operate,item,address);let proposal=loadObj(key);Utils.assert(proposal!==false,'Failed to get '+key+' from metadata.');Utils.assert(Chain.block.timestamp>=proposal.expiration&&proposal.passTime===undefined,'The proposal is still valid.');Chain.del(key);if(proposal.pledge>0){transferCoin(address,proposal.pledge,refundInput(),'refund');}}
