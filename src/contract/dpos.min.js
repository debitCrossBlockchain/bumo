...
function initialization(params){cfg=loadObj(configKey);Utils.assert(cfg===false,'Repeated initialization.');cfg={'gas_price':1000,'base_reserve':1000000,'committee_size':10,'kol_size':21,'kol_candidate_size':100,'kol_min_pledge':300000000000000,'validator_size':19,'validator_candidate_size':100,'validator_min_pledge':300000000000000,'pledge_magnification':2,'pass_rate':0.5,'valid_period':2592000000000,'vote_unit':1000000000,'reward_allocation_share':[50,8,35,7],'logic_contract':params.logic_contract};saveObj(configKey,cfg);Utils.assert(Utils.int64Compare(params.committee.length,cfg.committee_size)<=0,'The committee size is exceeded.');let i=0;for(i=0;i<params.committee.length;i+=1){Utils.assert(Utils.addressCheck(params.committee[i]),'Invalid address:'+params.committee[i]+'.');let proposalC=initProposal(role.COMMITTEE);saveObj(proposalKey(motion.APPLY,role.COMMITTEE,params.committee[i]),proposalC);}
