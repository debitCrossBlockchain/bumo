...
Utils.assert(Utils.addressCheck(pool),'Invalid address:'+pool+'.');Utils.assert(0<=ratio&&ratio<=100&&ratio%1===0,'Invalid vote reward ratio:'+ratio+'.');proposal.rewardPool=pool;proposal.rewardRatio=Number(ratio);if(roleType===role.KOL){return proposal;}
...
if(ratio!==undefined){Utils.assert(0<=ratio&&ratio<=100&&ratio%1===0,'Invalid vote reward ratio:'+ratio+'.');proposal.rewardRatio=Number(ratio);dist[Chain.msg.sender][2]=proposal.rewardRatio;}
