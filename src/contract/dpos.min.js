...
calculate(reward);distributed=true;elect.allStake=elect.balance;saveObj(stakeKey,elect.allStake);Chain.tlog('rewardDistribution',reward,Chain.msg.sender);}
function rewardInput(){return JSON.stringify({'method':'reward'});}
function award(candidates,dist,address){if(dist[address]==='0'){return;}
if(dist[address][2]===0){transferCoin(address,dist[address][0],rewardInput());Chain.tlog('award',address,dist[address][0],dist[address][1],'0');}
else if(dist[address][2]===100){transferCoin(dist[address][1],dist[address][0],rewardInput());Chain.tlog('award',address,'0',dist[address][1],dist[address][0]);}
else{let onePercent=Utils.int64Div(dist[address][0],100);let dividend=Utils.int64Mul(onePercent,dist[address][2]);transferCoin(dist[address][1],dividend,rewardInput());let reserve=Utils.int64Sub(dist[address][0],dividend);transferCoin(address,reserve,rewardInput());Chain.tlog('award',address,reserve,dist[address][1],dividend);}
...
function updateStake(roleType,candidate,formalSize,amount){let candidates=roleType===role.VALIDATOR?elect.valCands:elect.kolCands;let oldPos=candidates.indexOf(candidate);candidate[1]=Utils.int64Add(candidate[1],amount);candidates.sort(doubleSort);let newPos=candidates.indexOf(candidate);Chain.tlog('updateStake',candidate[0],roleType,amount);let key=roleType===role.VALIDATOR?valCandsKey:kolCandsKey;saveObj(key,candidates);if((oldPos>=formalSize&&newPos<formalSize)||(oldPos<formalSize&&newPos>=formalSize)){rewardDistribution();if(roleType===role.VALIDATOR){updateValidators(candidates);}}}
...
function setNodeAddress(address){Utils.assert(Utils.addressCheck(address),'Invalid address:'+address+'.');let key=proposalKey(motion.APPLY,role.VALIDATOR,Chain.msg.sender);let proposal=loadObj(key);Utils.assert(proposal!==false,Chain.msg.sender+' has not applied to become a validator.');proposal.node=address;saveObj(key,proposal);let candidates=loadObj(valCandsKey);Utils.assert(candidates!==false,'Failed to get '+valCandsKey+' from metadata.');let found=candidates.find(function(x){return x[0]===Chain.msg.sender;});if(found===undefined){return false;}
found[2]=address;saveObj(valCandsKey,candidates);Chain.tlog('setNodeAddress',Chain.msg.sender,address);if(candidates.indexOf(found)<cfg.validator_size){updateValidators(candidates);}}
function setVoteDividend(roleType,pool,ratio){Utils.assert(roleValid(roleType),'Unknown role:'+roleType+'.');let key=proposalKey(motion.APPLY,roleType,Chain.msg.sender);let proposal=loadObj(key);Utils.assert(proposal!==false,'Failed to get '+key+' from metadata.');let rewardKey=roleType===role.VALIDATOR?valRewardKey:kolRewardKey;let dist=loadObj(rewardKey);Utils.assert(dist!==false,'Failed to get '+rewardKey+' from metadata.');if(pool!==undefined){Utils.assert(Utils.addressCheck(pool),'Invalid address:'+pool+'.');proposal.rewardPool=pool;dist[Chain.msg.sender][1]=pool;}
if(ratio!==undefined){Utils.assert(0<=ratio&&ratio<=100&&ratio%1===0,'Invalid vote reward ratio:'+ratio+'.');proposal.rewardRatio=ratio;dist[Chain.msg.sender][2]=ratio;}
saveObj(key,proposal);saveObj(rewardKey,dist);Chain.tlog('setVoteDividend',pool||proposal.rewardPool,ratio||proposal.rewardRatio);}
...
else if(input.method==='getValidators'){let valCands=loadObj(valCandsKey);Utils.assert(valCands!==false,'Failed to get '+valCands+' from metadata.');result.validators=valCands.slice(0,cfg.validator_size);}
...
saveObj(valCandsKey,validators.sort(doubleSort));saveObj(kolCandsKey,[]);saveObj(valRewardKey,dist);saveObj(kolRewardKey,{});saveObj(stakeKey,Chain.getBalance(Chain.thisAddress));Chain.tlog('init',Chain.tx.sender,Chain.thisAddress,cfg.logic_contract);}
